#!/usr/bin/env python3
"""
Final import script - handles duplicates by dropping shot_id constraint
"""
...
try:
    result = supabase.table('nhl_shots_2024').delete().gte('shot_id', 0).execute()
...
# Check for duplicates in shot_id
duplicates = df['shotID'].duplicated().sum()
print(f"🔍 Found {duplicates} duplicate shot_id values")

if duplicates > 0:
    print("🧹 Removing duplicates...")
    df = df.drop_duplicates(subset=['shotID'], keep='first')
...
print(f"📊 Successfully imported {success_count:,} NHL shot records!")

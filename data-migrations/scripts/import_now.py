#!/usr/bin/env python3
"""
Simple CSV import to Supabase - Edit the credentials below
"""
...
if SUPABASE_KEY == "YOUR_ANON_KEY_HERE":
    print("❌ Please edit the script and add your Supabase anon key")
    print("Find it at: Supabase Dashboard -> Settings -> API -> anon/public key")
    sys.exit(1)
...
try:
    ...
    import os
    os.system("pip3 install pandas supabase numpy")
    import pandas as pd
    from supabase import create_client
    import numpy as np
...
# Clean column names - comprehensive mapping
column_mapping = {
    'shotID': 'shot_id',
...
for i in range(0, total_rows, batch_size):  # Import all rows
    batch = df.iloc[i:i+batch_size]
    batch_dict = batch.to_dict('records')
    
    try:
        result = supabase.table('nhl_shots_2024').insert(batch_dict).execute()
        print(f"✅ Batch {i//batch_size + 1}: Uploaded {len(batch_dict)} rows")
    except Exception as e:
        print(f"❌ Error: {e}")
        print("This might be due to column mismatch. Check your table schema.")
        break
